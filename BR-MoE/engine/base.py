...
from ..models.base import BaseBRMoEModel
import torch
...
class BRMoEWrapper:
    ...
    def _is_quantizable(cls, model):
        return hasattr(model, "brmoe_compressed")
    ...
    def _check_if_already_quantized(cls, model):
        assert not model.brmoe_compressed, "Model already quantized"

    @classmethod
    def _check_if_not_quantized(cls, model):
        assert model.brmoe_compressed, "Model not quantized."
    ...
    def from_quantized(
        ...
        save_dir = BaseBRMoEModel.try_snapshot_download(save_dir_or_hub, cache_dir=cache_dir)
